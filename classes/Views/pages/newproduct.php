...
<form id="newproductform" class="formulario-new-produto" method="post">
    ...
    <div class="base-form">
        ...
        <div class="dados-right-form">
            ...
            <?php 
            if(isset($arr[1]['type']) AND $arr[1]['type'] == "recarga"){ ?>

            <?php }else{ ?>
            <div class="space-input space-tri-remove">
                ...
                if(isset($arr[1]) && $arr[1]['type'] == "recarga"){
                    ...
                    foreach ($arr[4] as $key => $varia) { ?>
                        ...
                        <div id="body-variacao-space1" class="corpo-das-variacoes corpo-das-variacoes-item<?= $varia['id'] ?> color-red-backgorund"><div class="body-variacao-space">
                                ...
                                <?php 
                                    if(isset($arr[1]['type']) AND $arr[1]['type'] == "recarga"){ ?>
                                        <input type="tel" class="moneyrs" value="<?= \Painel::convertMoney($varia['preco']) ?>" name="atuprecovariacao[]" placeholder="valor">
                                        <input type="tel" class="moneyrs" value="<?= \Painel::convertMoney($varia['value_promotion']) ?>" name="atuprecopromotionvariacao[]" placeholder="valor">
                                    <?php } ?>
...
<input id="fotoinput" class="input-file-foto" type="file" name="foto[]"  multiple >
    ...
    <?= (isset($arr[1]))? '<input data-url="'.URL_LOJA.'" class="btn_padrao btn-editar-produto" type="submit" name="action" value="Editar produto">' : '<input data-url="'.URL_LOJA.'" class="btn-criar-produto" type="submit" name="action" value="Criar produto">' ?>
