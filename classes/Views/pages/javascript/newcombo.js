...
//     const typeselect = $(this).val();
//     const inputs = '<div id="body-variacao-space'+cont+'" class="corpo-das-variacoes"><div  class="body-variacao-space"><input type="text" name="nomevariacao[]" placeholder="Tv 3 meses"><select name="typevariacao[]"><option disabled selected>Tipo do código</option><option value="mensal">Mensal</option><option value="anual">Anual</option></select><input type="tel" name="qtdvariacao[]" placeholder="Quantidade de itens"><select name="plataforma[]"><option disabled selected>Plataforma</option><option disabled selected>Qual plataforma</option><option value="mfc">My family cinema</option><option value="tvpix">Tv Pix</option><option value="bluetv">Blue tv</option><option value="redplay">Red play</option><option value="tvexpress">Tv express</option></select></div><div class="space-delete"><i id="'+cont+'" class="delete-inputs bx bx-trash" ></i></div></div>';
...
$('body').on('click', '#atu-delete-varia', function (e) {
    ...
    $.ajax({
        method: "post",
        url: 'newcombo/sessiondeletevaria',
...
$('body').on('click', '.btn-editar-combo', function (e) {
    // $( ".formulario-new-produto" ).submit(function( e ) {
    e.preventDefault();
